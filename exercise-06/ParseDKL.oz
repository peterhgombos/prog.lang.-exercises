...
local
   ...
   fun {DefProc Tokens Rest}
      try
          local Rest1 in
            '{'|'$'|'proc'({Ident Tokens 'then'|Rest1})|'}' {Stat Rest1 'end'|Rest}
          end
      catch E then raise procPE(E) end
   end
   ...
   fun {If Tokens Rest}
      try
          local Rest1 Rest2 in
              'cond'({Ident Tokens 'then'|Rest2} {Stat Rest1 'else'|Rest2} {Stat 'end'|Rest})
          end
      catch E then raise condPE(E) end
   end
   ...
   fun {Case Tokens Rest}
      try
          local Rest1 Rest2 Rest3 in
              'case'({Ident Tokens 'of'|Rest3} {Pat Rest3 'then'|Rest2} {Stat Rest2 'else'|Rest1} {Stat Rest1 'end'|Rest})
          end
      catch E then raise casePE(E) end
   end
   fun {Pat Tokens Rest}
      local Rest1 in
         recpat( {Lit Tokens Rest1}
   ...
   fun {Application Tokens Rest}
      try
          local Rest1 in
              proc_app('{'|{Ident Tokens Rest1} {IdentList Rest1 '}'|Rest}})
          end
      catch E then raise applicatonPE(E) end
   end
